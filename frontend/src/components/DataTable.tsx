            ...
            <table className="w-full">
                    ...
                    <tr className="border-b border-slate-700/50">
                        {columns.map((column) => (
                            <th
                                ...
                                className={`px-4 py-3 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider ${column.className || ""}`}
                            >
                                {column.header}
                            </th>
                        ))}
                ...
                <tbody className="divide-y divide-slate-800/50">
                    {data.map((item, index) => (
                        ...
                        >
                            {columns.map((column) => (
                                <td key={String(column.key)} className={`px-4 py-3 text-sm text-slate-300 ${column.className || ""}`}>
                                    {column.render
                                        ? column.render(item)
